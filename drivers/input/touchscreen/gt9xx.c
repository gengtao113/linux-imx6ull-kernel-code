...
static void gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
    ...
    input_report_abs(ts->pen_dev, ABS_MT_POSITION_X, ts->abs_x_max - x);
    input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y, ts->abs_y_max - y);
...
static s8 gtp_i2c_test_no_rst(struct i2c_client *client)
{
    ...
    while(retry++ < 5)
    {
        ret = gtp_i2c_read_no_rst(client, test, 3);
        if (ret > 0)
        {
            return ret;
        }
...
#if GTP_ESD_PROTECT
    ts->clk_tick_cnt = 10 * HZ;      // HZ: clock ticks in 1 second generated by system
